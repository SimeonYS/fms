...
class FmsPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `fms`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM fms WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO fms (date, title, link, content)"
